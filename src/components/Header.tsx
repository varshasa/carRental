...
const Header:React.FC = () => {
  const [isMoon, setIsMoon] = useState<boolean>(false); // State to track the icon

  const toggleIcon = () => {
    setIsMoon((prev) => !prev); // Toggle between true and false
  };
  return (
    <>
      ...
      >
        ...
        <div className="hidden lg:text-sm text-gray-400"><img className="w-5 inline mr-2" src={location}/>New York , US</div>
        <div className="lg:text-sm"><img className="w-10 inline mr-2" src={profile}/><span className="hidden">Nancy Holloway</span></div>
      </div>
    </>
  )
}
